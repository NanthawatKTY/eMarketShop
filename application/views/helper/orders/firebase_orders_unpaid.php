...
window.onload = async function(){
            ...
            baseDoc.onSnapshot(function(snapshot) {
                snapshot.data().orderList.forEach(function(change) {
                    if (change.isActive == true){
                        showOrdersUnpaid(change);
                    }  
                });
            });     

            $( "#shippingSort2" ).on('change',function() {
                getOrdersUnpaidByShipping( $(this).val());
            });
    ...
    $(document).ready(function () {
        ...
        $("#chat-search").on("keyup", function () {
            ...
            if (value != "") {
                $(".tbBodyUnpaid tr").filter(function () {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
                });
            } else {
                // if search filter box is empty
                $(".tbBodyUnpaid tr").show();
            }
        });
    });
    ...
    function getOrdersUnpaidByShipping(shippingSort){
    ...
    function showOrdersUnpaid(data){
